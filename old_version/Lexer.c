...
void print_token_table(Lexer *lexer){
    ...
    for(int i = 0; i < lexer->token_count; i++){
        ...
        //if(lexer->token_table[i].token_type == TOKEN_T_){ // DEBUG
...
int final_state_end_of_line(Lexer *lexer){
    lexer->current_row++;
    lexer->current_col = 1;
    Token *token = create_token(TOKEN_T_EOL, lexer->lexeme, lexer->lexeme_length, lexer->current_row, lexer->current_col, lexer->scope);
...
int final_state_comma(Lexer *lexer){
    Token *token = create_token(TOKEN_T_COMMA, lexer->lexeme, lexer->lexeme_length, lexer->current_row, lexer->current_col, lexer->scope);
    add_token_to_token_table(lexer, token);
    lexer_start(lexer);
}

int final_state_brackets(Lexer *lexer){
    Token *token = create_token(TOKEN_T_BRACKET, lexer->lexeme, lexer->lexeme_length, lexer->current_row, lexer->current_col, lexer->scope);
    add_token_to_token_table(lexer, token);
    lexer_start(lexer);
}
...
int final_state_identif(Lexer *lexer){
    Token *token = create_token(TOKEN_T_IDENTIFIER, lexer->lexeme, lexer->lexeme_length, lexer->current_row, lexer->current_col, lexer->scope);
...
int final_state_keyword(Lexer *lexer){
    Token *token = create_token(TOKEN_T_KEYWORD, lexer->lexeme, lexer->lexeme_length, lexer->current_row, lexer->current_col, lexer->scope);
    add_token_to_token_table(lexer, token);
    lexer_start(lexer);
}
...
int final_state_global_identif(Lexer *lexer){
    Token *token = create_token(TOKEN_T_GLOBAL_VAR, lexer->lexeme, lexer->lexeme_length, lexer->current_row, lexer->current_col, lexer->scope);
...
int final_state_number(Lexer *lexer){
    Token *token = create_token(TOKEN_T_NUM, lexer->lexeme, lexer->lexeme_length, lexer->current_row, lexer->current_col, lexer->scope);
    //Symbol *symbol = lexer_create_num_literal_sym_from_token(token);
    add_token_to_token_table(lexer, token);
    //insert_into_symtable(lexer->symtable, symbol);
    lexer_start(lexer);
};
...
int final_state_string(Lexer *lexer){
    ...
    if(strcmp(lexer->lexeme, "\"\"") == 0 && lexer->current_char == '"'){
        ...
        Token *token = create_token(TOKEN_T_STRING, lexer->lexeme, lexer->lexeme_length, lexer->current_row - lexer->newlines_in_multiline, lexer->current_col, lexer->scope);
        //Symbol *symbol = lexer_create_string_literal_sym_from_token(token);
        add_token_to_token_table(lexer, token);
        //insert_into_symtable(lexer->symtable, symbol);
        lexer->newlines_in_multiline = 0;
...
int final_state_comment(Lexer *lexer){
    Token *token = create_token(TOKEN_T_EOL, "\n", 1, lexer->current_row, lexer->current_col, lexer->scope);
    add_token_to_token_table(lexer, token);
    lexer->current_row++;
    lexer_start(lexer);
}
...
int final_state_operator(Lexer *lexer){
    Token *token = create_token(TOKEN_T_OPERATOR, lexer->lexeme, lexer->lexeme_length, lexer->current_row, lexer->current_col, lexer->scope);
    add_token_to_token_table(lexer, token);
    lexer_start(lexer);
}
